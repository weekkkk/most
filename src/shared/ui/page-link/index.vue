...
<template>
  <li
    class="max-md:text-[0.7rem] page_link group list-none inline-flex flex-col"
  >
    <button
      class="page_link-header bg-brand-100 text-default w-full md:p-[20px] max-md:p-3 md:leading-control max-md:leading-[0.25rem] md:rounded-t-[calc(10px+1.2rem)] md:rounded-b-[calc(10px+1.2rem)] max-md:rounded-t-[0.875rem] max-md:rounded-b-[0.875rem] group-hover:rounded-b-none transition-all duration-150 group-hover:delay-0"
    >
    ...
    <div v-if="children" :class="{ 'max-h-0': absolute }">
      <div
