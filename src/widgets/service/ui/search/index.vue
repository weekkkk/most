...
<template>
  <UiButton
    id="searchButton"
    @click="openSearch()"
    class="!p-[1.45rem] max-2xl:!p-[1.15rem] max-md:!p-[.65rem] !rounded-[2.2rem] flex !justify-between w-[4.4rem] max-2xl:w-[3.8rem] max-md:w-[2.5rem] cursor-pointer transition-all duration-300"
    :class="{
  ...
  >
    ...
    <input
      ...
      :value="searchText"
      @input="search"
      class="-my-8 h-[4.4rem] max-2xl:h-[3.8rem] max-md:h-[2.5rem] w-full text-[2rem] max-2xl:text-[1.5rem] max-md:text-[1rem] leading-small ml-[1rem] max-md:ml-[0.5rem] font-medium text-default outline-none"
